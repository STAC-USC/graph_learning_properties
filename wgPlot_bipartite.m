function [hE,hV]=wgPlot_bipartite(adjMat,coord,Vsets,varargin)
...
plotParm={'markerSize',20,'lineWidth',0.1,'marker','none'};
siz=size(adjMat);
vrtxSiz=100;
edgeMap=cool;
vrtxMap=summer;
...
if eWeighted
  ...
  text([legLim(1),legLim(end)],[legY-0.5*yStep,legY-0.5*yStep], ...
    {num2str(minEWt),num2str(maxEWt)},'HorizontalAlignment','center','FontSize',14,'FontName','Times New Roman');
else
    nSegment=length(ii);
...
if vWeighted
  ...
  if exist('vrtxCol','var')
    ...
    hV=scatter(coord(Vsets{1},1),coord(Vsets{1},2),vWt,'filled','MarkerFaceColor',[1,0,0]);
    hV=scatter(coord(Vsets{2},1),coord(Vsets{2},2),vWt,'filled','MarkerFaceColor',[0,0,0]);
  end
